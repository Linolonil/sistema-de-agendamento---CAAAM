...
const registerUser = async (req, res) => {
  try {
    ...
    const user = new User({ name,userName: userName.toLowerCase(), role, password: hashedPassword });
    await user.save();
...
const loginUser = async (req, res) => {
  try {
    ...
    // Gerar um token JWT
    const token = jwt.sign({ id: user._id, username: user.userName,  role: user.role, name: user.name }, process.env.JWT_SECRET, { expiresIn: '1d' });
